<script>
  ...
  $: activeCategory = sortedCategories[0]?.id; // Para tracking de categoría activa

  // Función para scroll suave a la categoría
  ...
  const whatsappNumber = "5492257693350"; // Reemplazar con el número real
  ...
  {#each sortedCategories as category}
    <div id="category-{category.id}" class="mb-8">
      ...
      <div class="grid gap-4">
        {#each menu.menuItems.filter(item => item.category === category.name) as item}
              ...
              <div class="flex gap-4">
                ...
                {#if item.image}
                  <a href="/images/{item.image}" 
                     class="block w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
